<x-frontend-layout>
    ...
    <section class="py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($years->count() > 0)
            <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-6 mb-8 flex flex-wrap items-center gap-2 border border-gray-100">
                <span class="text-gray-700 font-semibold mr-2 font-heading">Filter Tahun:</span>
                <a href="{{ request()->url() }}" class="px-5 py-2 rounded-full font-medium text-sm transition-all duration-300 {{ !request('year') ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-600/30 transform scale-105' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 hover:text-gray-900' }}">Semua</a>
                @foreach($years as $year)
                <a href="{{ request()->fullUrlWithQuery(['year' => $year]) }}" class="px-5 py-2 rounded-full font-medium text-sm transition-all duration-300 {{ request('year') == $year ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-600/30 transform scale-105' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 hover:text-gray-900' }}">{{ $year }}</a>
                @endforeach
            </div>
            @endif
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-semibold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">{{ $report->year }}</span>
                                @if($type === 'keuangan_publikasi')
                                <span class="text-xs font-semibold text-blue-700 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100">{{ $report->quarter ? 'Q'.$report->quarter : 'Tahunan' }}</span>
                                @endif
                            </div>
                        </div>
                        <h3 class="text-lg font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-emerald-600 transition-colors">{{ $report->title }}</h3>
                        @if($report->description)
                        <p class="text-sm text-gray-600 mb-4">{{ Str::limit($report->description, 90) }}</p>
                        @endif
                        ...
                        <div class="flex items-center gap-3 mt-4">
                            <span class="flex items-center text-blue-600 bg-blue-50 px-2 py-1 rounded-full text-xs font-medium">
                            ...
                            <span class="flex items-center text-green-600 bg-green-50 px-2 py-1 rounded-full text-xs font-medium">
                        ...
                        <div class="flex items-center gap-2 mt-6">
                            <button type="button" onclick="openPreview('{{ route('reports.preview', $report->id) }}', '{{ e($report->title) }}', {{ $report->id }})" class="flex-1 px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm rounded-lg hover:bg-gray-50 hover:text-blue-600 transition-colors duration-200 flex items-center justify-center shadow-sm">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                Preview
                            </button>
                            <a href="{{ route('reports.download', $report->id) }}" onclick="incrementDownload({{ $report->id }})" class="flex-1 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-sm rounded-lg hover:shadow-lg hover:shadow-emerald-600/30 transition-all duration-200 flex items-center justify-center shadow-md">
