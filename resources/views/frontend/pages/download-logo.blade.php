<x-frontend-layout>
    ...
    <section class="py-12 sm:py-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 p-6 sm:p-8">
                @if($logoAvailable)
                    <!-- Logo Preview -->
                    <div class="text-center mb-6 sm:mb-8">
                        <div class="w-48 h-48 sm:w-64 sm:h-64 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl sm:rounded-2xl mx-auto flex items-center justify-center mb-4 sm:mb-6 p-6 sm:p-8 border border-gray-200">
                            <img src="{{ \App\Helpers\StorageHelper::url($company->logo) }}" alt="{{ $company->name ?? 'Logo' }}" class="max-w-full max-h-full object-contain">
                        </div>
                        <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Logo {{ $company->name ?? 'BPRS Bangka Belitung' }}</h2>
                        <p class="text-sm sm:text-base text-gray-500 mt-2">Format tersedia: {{ strtoupper($logoExtension) }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
                        ...
                        <a href="{{ route('download-logo.download', $logoExtension) }}" 
                           class="group flex items-center justify-center px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl hover:from-emerald-700 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-emerald-500/30 touch-manipulation active:scale-95 min-h-[48px] text-sm sm:text-base">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 group-hover:animate-bounce flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('download-logo.download', 'png') }}" 
                           class="group flex items-center justify-center px-4 sm:px-6 py-3 sm:py-4 border-2 border-emerald-600 text-emerald-600 rounded-xl hover:bg-emerald-50 transition-all duration-300 touch-manipulation active:scale-95 min-h-[48px] text-sm sm:text-base">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 group-hover:animate-bounce flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                            </svg>
                ...
                <!-- Usage Guidelines -->
                <div class="mt-6 sm:mt-8 p-4 sm:p-6 bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl border border-amber-200">
                    <div class="flex items-start gap-3 sm:gap-4">
                        ...
                        <div>
                            <h3 class="font-semibold text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">Panduan Penggunaan Logo</h3>
