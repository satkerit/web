<x-frontend-layout>
    ...
    <!-- Prayer Widget - Responsive Design -->
    <div x-data="{ 
        show: true, 
        minimized: false,
        isMobile: false,
        ...
        <div x-show="!isMobile && show"
             class="hidden xl:block fixed right-0 top-20 z-40 transition-all duration-300"
             :class="minimized ? 'translate-x-[calc(100%-2.5rem)]' : 'translate-x-0'"
             style="max-height: calc(100vh - 6rem);">
            
            <!-- Toggle Button -->
            <button 
                @click="minimized = !minimized"
                class="absolute left-0 top-3 -translate-x-full bg-gradient-to-r from-emerald-600 to-teal-700 text-white p-2 rounded-l-lg shadow-lg hover:shadow-xl transition-all"
                :class="minimized ? 'opacity-100' : 'opacity-0 hover:opacity-100'"
                title="Toggle Prayer Times">
                <svg class="w-4 h-4 transition-transform duration-300" :class="minimized ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </button>

            <!-- Widget Container -->
            <div class="w-80 overflow-y-auto prayer-widget-container shadow-2xl" style="max-height: inherit;">
                <x-prayer-time-widget />
            </div>
        </div>

        <!-- Mobile/Tablet Version: Bottom Sheet Modal -->
        <div x-show="isMobile && show"
             ...
             class="xl:hidden fixed inset-0 z-50 flex items-end sm:items-center justify-center"
            ...
            <div x-show="show"
                ...
                <button 
                    @click="show = false"
                    class="absolute -top-12 right-4 sm:right-0 w-10 h-10 bg-white rounded-full shadow-lg flex items-center justify-center hover:bg-gray-100 transition-colors z-10"
                    title="Close">
                ...
                <!-- Widget Container with max height for mobile -->
        ...
        <button 
            x-show="isMobile && !show"
            x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="scale-0 opacity-0"
            x-transition:enter-end="scale-100 opacity-100"
            @click="show = true"
            class="xl:hidden fixed bottom-20 right-4 w-14 h-14 bg-gradient-to-r from-emerald-600 to-teal-700 text-white rounded-full shadow-lg hover:shadow-xl flex items-center justify-center transition-all duration-300 hover:scale-110 z-40 group"
            title="Jadwal Sholat">
