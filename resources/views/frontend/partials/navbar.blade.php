...
<nav class="relative" x-data="{ scrolled: false, mobileOpen: false, ready: false }" x-init="$nextTick(() => { ready = true; scrolled = window.scrollY > 50 })" @scroll.window="scrolled = window.scrollY > 50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            ...
            <a href="{{ route('home') }}" class="flex items-center space-x-3 group">
                @if($company?->logo)
                <div class="bg-white rounded-xl p-2 shadow-md group-hover:shadow-lg transition-all duration-300">
                    <img src="{{ Storage::url($company->logo) }}" alt="{{ $company->name ?? 'Logo' }}" class="h-10 w-auto group-hover:scale-105 transition-all duration-300">
                </div>
                @else
            ...
            <div class="hidden lg:flex items-center space-x-1">
                <a href="{{ route('home') }}" class="nav-link px-4 py-2 rounded-lg font-medium transition-all duration-300" :class="scrolled ? 'text-gray-800 hover:text-emerald-600 hover:bg-emerald-50' : 'text-gray-800 hover:text-emerald-600 hover:bg-gray-100/30'">
                    Beranda
                </a>
                ...
                <div class="relative" x-data="{ open: false, timeout: null }"
                    ...
                    <div x-cloak x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" class="absolute left-0 top-full pt-1 w-64 z-50">
                        <div class="bg-white rounded-2xl shadow-xl py-3 border border-gray-100">
            ...
            <!-- Mobile menu button -->
            <button @click="mobileOpen = !mobileOpen" class="lg:hidden relative z-50 p-3 rounded-xl transition-all duration-300 shadow-lg" :class="scrolled ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-emerald-500/30' : 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-emerald-500/30'">
                <span class="sr-only">Toggle menu</span>
                <div class="w-6 h-5 relative flex flex-col justify-center">
                    <span x-show="!mobileOpen" class="block absolute h-0.5 w-6 transform transition duration-300 ease-in-out" :class="scrolled ? 'bg-white' : 'bg-white'" :style="mobileOpen ? 'rotate: 45deg; translate: 0px, 0px;' : 'rotate: 0deg; translate: 0px, -8px;'"></span>
                    <span x-show="!mobileOpen" class="block absolute h-0.5 w-6 transform transition duration-300 ease-in-out" :class="scrolled ? 'bg-white' : 'bg-white'" :style="mobileOpen ? 'rotate: -45deg; translate: 0px, 0px;' : 'rotate: 0deg; translate: 0px, 0px;'"></span>
                    <span x-show="!mobileOpen" class="block absolute h-0.5 w-6 transform transition duration-300 ease-in-out" :class="scrolled ? 'bg-white' : 'bg-white'" :style="mobileOpen ? 'opacity: 0; translate: 0px, 0px;' : 'rotate: 0deg; translate: 0px, 8px;'"></span>
                </div>
            </button>
        </div>
    </div>
    ...
    <div x-cloak x-show="mobileOpen"
         ...
         class="lg:hidden fixed top-20 left-0 right-0 w-full h-full bg-white shadow-2xl z-50 overflow-y-auto mobile-nav-scroll">

        <!-- Mobile Header -->
        <div class="sticky top-0 bg-white border-b border-gray-200 p-4 shadow-sm">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    <div>
                        <h3 class="font-semibold text-lg text-gray-800">Menu</h3>
                        <p class="text-gray-500 text-sm">{{ $company?->name ?? 'BPRS' }}</p>
                    </div>
                </div>
                <button @click="mobileOpen = false" class="p-2 rounded-lg hover:bg-gray-100 transition-colors">
        ...
        <div class="p-4 space-y-3">
            <!-- Home -->
            <a href="{{ route('home') }}" @click="mobileOpen = false" class="flex items-center px-4 py-3 text-gray-700 hover:bg-emerald-50 hover:text-emerald-600 rounded-xl transition-all duration-200 group">
                <span class="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center mr-3 group-hover:bg-emerald-200 transition-colors">
            ...
            <div x-data="{ open: false }">
                <button @click="open = !open" class="w-full flex justify-between items-center px-4 py-3 text-gray-800 hover:bg-emerald-50 rounded-xl font-medium transition-all duration-200 group">
                    <div class="flex items-center">
                        <span class="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mr-3 group-hover:bg-blue-200 transition-colors">
                ...
                <div x-cloak x-show="open" x-collapse class="ml-13 space-y-1 mt-2 pl-4 border-l-2 border-emerald-200">
                    <a href="{{ route('about.company') }}" @click="mobileOpen = false" class="flex items-center px-4 py-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <span class="w-2 h-2 bg-emerald-400 rounded-full mr-3"></span>
                        Profil Perusahaan
                    </a>
                    <a href="{{ route('about.komisaris') }}" @click="mobileOpen = false" class="flex items-center px-4 py-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <span class="w-2 h-2 bg-emerald-400 rounded-full mr-3"></span>
                        Dewan Komisaris
                    </a>
                    <a href="{{ route('about.direksi') }}" @click="mobileOpen = false" class="flex items-center px-4 py-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <span class="w-2 h-2 bg-emerald-400 rounded-full mr-3"></span>
                        Dewan Direksi
                    </a>
                    <a href="{{ route('about.pengawas-syariah') }}" @click="mobileOpen = false" class="flex items-center px-4 py-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <span class="w-2 h-2 bg-emerald-400 rounded-full mr-3"></span>
                        Dewan Pengawas Syariah
                    </a>
                    <a href="{{ route('about.struktur') }}" @click="mobileOpen = false" class="flex items-center px-4 py-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <span class="w-2 h-2 bg-emerald-400 rounded-full mr-3"></span>
                        Struktur Organisasi
                    </a>
                    <a href="{{ route('about.offices') }}" @click="mobileOpen = false" class="flex items-center px-4 py-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <span class="w-2 h-2 bg-emerald-400 rounded-full mr-3"></span>
                        Kantor Kami
                    </a>
                </div>
            ...
            <div class="pt-4 border-t border-gray-200 mt-4">
                <a href="{{ route('contact') }}" @click="mobileOpen = false" class="block w-full px-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-center rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
