...
<x-admin.card :noPadding="true">
        ...
        <form method="GET" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @if(request('search') || request('status') || request('type'))
                    <a href="{{ route('admin.complaints.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-slate-600 bg-white rounded-lg ring-1 ring-inset ring-slate-200 hover:bg-slate-50 transition-colors">
                        Reset
                    </a>
                @endif
            </div>
    ...
    <div class="block md:hidden p-4 space-y-4">
        @forelse($complaints as $complaint)
            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <div class="mb-3">
                    ...
                    <div class="text-sm">
                        @if($complaint->is_anonymous)
                            <span class="text-gray-400 italic">Pelapor: Anonim</span>
                        @else
                ...
                <div class="flex items-center gap-2 pt-3 border-t border-gray-100">
                    <a href="{{ route('admin.complaints.show', $complaint) }}" class="flex-1 text-center py-2 text-sm font-medium text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        Lihat Detail
                    </a>
                    <form action="{{ route('admin.complaints.destroy', $complaint) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus pengaduan ini?')" class="flex-1">
                        @csrf @method('DELETE')
        ...
        @empty
            <div class="text-center py-8 text-gray-500">Belum ada pengaduan.</div>
        @endforelse
    </div>
    ...
    <div class="hidden md:block">
        <x-admin.table :headers="['Tiket', 'Pelapor', 'Tipe', 'Status', 'Tanggal', 'Aksi']">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 text-sm">
                        ...
                        @else
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-gray-500 truncate">{{ $complaint->email }}</p>
                            </div>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">
                        <div class="flex items-center gap-1">
                            <a href="{{ route('admin.complaints.show', $complaint) }}" class="p-1.5 text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                            </a>
                            <form action="{{ route('admin.complaints.destroy', $complaint) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus pengaduan ini?')">
                                @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">Belum ada pengaduan.</td></tr>
            @endforelse
        </x-admin.table>
    </div>
